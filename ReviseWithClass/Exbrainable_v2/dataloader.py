...
class DataLoader():
    def __init__(self, FT, sch_var):

        self.FT = FT
        self.dev = torch.device(f"cuda:0") if torch.cuda.is_available() else torch.device("cpu")
        self.sch_var = sch_var

    def Individual_Dataset(self):
        bs= self.sch_var.bs
        ratio= self.sch_var.val_ratio
        situation= self.sch_var.situation
            
        if (situation==0 or situation==1 or situation==2):

            x_T= torch.Tensor(self.sch_var.x_train).unsqueeze(1)
            y_T= torch.Tensor(self.sch_var.y_train).view(-1)
            x_train, y_train, x_val, y_val= self.train_val_split(x_T, y_T, ratio)

            x_train= x_train.to(self.dev)
            y_train= y_train.long().to(self.dev)
            x_val= x_val.to(self.dev)
            y_val= y_val.long().to(self.dev)
            ...
            if (situation==1 or situation==2): 
                x_test= torch.Tensor(self.sch_var.x_test).unsqueeze(1)
                ...
                if situation==2 : 
                    y_test= torch.Tensor(self.sch_var.y_test).view(-1)
                    y_test= y_test.long().to(self.dev)
    ...
    def train_val_split(self, x_T, y_T, ratio):
        s = y_T.argsort() # sort and split the four classes
        x_T = x_T[s]
        y_T = y_T[s]
