...
class Visualization():
    ...
    def new_window(self, Title, size, x_pad, y_pad):
        win_main_x = self.sch_var.win_main.winfo_rootx()+ x_pad
        win_main_y = self.sch_var.win_main.winfo_rooty()+ y_pad
    ...
    def montage_select(self, modelframe, Montagename):
        w = self.new_window('montage selection', '500x200', x_pad=-8, y_pad=150 )
        ...
        Confirm = ttk.Button(w ,text="Confirm", command=(lambda:[self.channel_select(w, modelframe, stdmontagelist, Montagename)]))
        stdmontagelist.pack()
        Confirm.pack()


    def channel_select(self, w, modelframe, stdmontagelist, Montagename):
        ...
        w2 = self.new_window('Channel selection', '500x300', x_pad=-8, y_pad=150)
        chlist = tk.Listbox(w2, selectmode=tk.EXTENDED)
        for c in chs:
            chlist.insert(tk.END, c)

        Confirm = ttk.Button(w2 ,text="Confirm", command=(lambda:self.select_ch(w2, modelframe, name, chlist,montage, Montagename)))

        chlist.pack()
        Confirm.pack()


    def select_ch(self, w2, modelframe, name, chlist, montage, Montagename):
        ch_select = chlist.curselection()
        if len(ch_select) != self.sch_var.ch:
            tk.messagebox.showerror('Error', f'Number of channel must be equal to {self.sch_var.ch}.', parent=w2)
        else:    
            electrode = []
            for c in ch_select:
            ...
            w2.destroy()
            self.Spatial_kernel(montage, electrode)


    def Spatial_kernel(self, montage, electrode):
    ...
    def Confusion_Matrix(self, w, y_true, y_pred):
        ...
        ax.set_xticks(np.arange(self.sch_var.n_class))
        ax.set_yticks(np.arange(self.sch_var.n_class))
